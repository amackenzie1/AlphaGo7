...
def clean(board, priority):
    ...
    def kids(position):
        potentials = [position + 1, position - 1, position + 7, position - 7]
        potentials = [i for i in potentials if i >= 0 and i < 49 and abs(position % 7 - i % 7) < 2]
        return potentials
    ...
    def get_connected_components(board, who):
        ccs = []
        all_positions = list(range(49))
        for i in list(all_positions):
            if board[i//7][i%7] != who:
                all_positions.remove(i)
...
class Board:
    ...
    def from_numbers(self, move):
        return move//7, move%7

    def move(self, move, who):

        if move == 49 or str(move).lower() == "pass":
            self.moves += 1
